...
public class CanvasEraserPen extends CanvasPen {
    ...
    /**
     * Computes the new bounds with the current touch point in the center and
     * a given side-length
     *
     * @param eraserPosition The current touch position
     * @param eraserRadius   The side-length of the bounds
     */
    private void computeEraserBounds(Vector2f eraserPosition,
                                     float eraserRadius) {
        ...
        mEraserBounds.offset(-mEraserBounds.width() / 2,
                -mEraserBounds.height() / 2);
    ...
    public int erase(Vector2f eraserPosition, float eraserRadius) {
        ...
        for (int i = 0; i < strokes.size(); i++) {
            ...
            //check if the bounding box intersects the bounds, if not then
            // the stroke itself will not either
            if (SAT.rectangleRectangleIntersection(rectPts, boundsPts)) {

                ArrayList<Float> points = strokes.get(i).getPathPoints();
                //check if the path defined by the points in the stroke
                // intersect with the bounds
                //if there is only one point (x, y) in the path check if that
                // point lies inside the bounds
                final boolean intersects =
                        (strokes.get(i).getPathPoints().size() == 2) ?
                        SAT.rectangularPointRectangleIntersection(
                                points.get(0), points.get(1),
                                strokes.get(i).getWeight(), rectPts) :
                ...
                if (intersects) {
                    ...
                    mCanvasWriterRef.getUndoRedoManager().
                            addAction(new CanvasWriterAction(
                                    CanvasWriterAction.Type.ERASE, erasedStroke));
                    strokesErased++;
                }
            }
        }
