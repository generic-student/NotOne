...
/**
 * Converts a CanvasView Object and all its associated Members to a
 * JSONObject for serialization
 *
 * @author Kai Titgens
...
public class CanvasExporter {
     ...
     * @param exportUndoTree True if the undo-redo tree should be included in
     *                      the export
     * @return JSONObject
     * @throws JSONException When the CanvasView cannot be converted to a
     * JSONObject
     */
    public static JSONObject canvasViewToJSON(@NonNull CanvasView view,
                                              boolean exportUndoTree) throws JSONException {
     ...
     * @param exportUndoTree True if the undo-redo tree should be incuded in
     *                       the export
     * @return JSONObject
     * @throws JSONException When the CanvasWriter cannot be converted to a
     * JSONObject
     */
    public static JSONObject canvasWriterToJSON(@NonNull CanvasWriter writer,
                                                boolean exportUndoTree) throws JSONException {
        ...
        List<JSONObject> actionsJSON =
                writer.getUndoRedoManager().getActions().stream().map(action -> {
        ...
        List<JSONObject> undoneActionsJSON =
                writer.getUndoRedoManager().
                        getUndoneActions().
                        stream().
                        map(action -> {
            final int strokeId = writer.getStrokes().indexOf(action.stroke);
     ...
     * @param strokeId the index of the stroke that is associated with the
     *                 action
     * @return JSONObject
     */
    public static JSONObject canvasWriterActionToJSON(CanvasWriterAction action,
                                                      int strokeId) {
        JSONObject json = new JSONObject();
        try {
    ...
    public static JSONObject canvasPdfDocumentToJson(CanvasPdfDocument document) {
        JSONObject json = new JSONObject();
        try {
            Bitmap[] pagesList = document.getPages();//document.getPages()
            // .length > 15 ? Arrays.copyOfRange(document.getPages(), 0, 14)
            // : document.getPages();
    ...
    public static JSONObject bitmapToJson(Bitmap bitmap) {
        ...
        encodedImage =
                Base64.encodeToString(byteArrayBitmapStream.toByteArray(),
                        Base64.DEFAULT);
