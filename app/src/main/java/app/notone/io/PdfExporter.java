...
public class PdfExporter {
    ...
    private static List<RectF> getStrokeBoundsSortedY(CanvasView view) {
        ...
        final List<RectF> bounds =
                view.getCanvasWriter().getStrokes().stream().map(stroke -> {
            RectF b = new RectF();
        ...
        final List<RectF> boundsSortedY =
                bounds.stream().sorted((b1, b2) -> Float.compare(b1.top,
                        b2.top)).collect(Collectors.toList());
     ...
     */
    @NonNull
    public static ArrayList<Rect> computePdfPageBoundsFromCanvasViewStrict(
            @NonNull CanvasView view, float dpi, @NonNull PageSize pageSize) {
        ...
        final List<RectF> boundsSortedY = getStrokeBoundsSortedY(view).
                ...
                collect(Collectors.toList());
        int maxBottom =
                (int) boundsSortedY.stream().
                        max((b1, b2) -> Float.compare(b1.bottom, b2.bottom)).
                        orElse(new RectF(0, 0, 0, 0)).bottom;

        //create a list for the bounding boxes
        ArrayList<Rect> pages = new ArrayList<>();
        for (int i = 0; i < pdfPages; i++) {
            pages.add(new Rect(0, i * heightPixels, widthPixels,
                    (i + 1) * heightPixels));
        }

        //compute the bounding boxes
        int pageIndex = pdfPages;
        Rect page = new Rect(0, pageIndex * heightPixels, widthPixels,
                (pageIndex + 1) * heightPixels);
        while (page.top < maxBottom) {
            ...
            pageIndex++;
            page = new Rect(0, pageIndex * heightPixels, widthPixels,
                    (pageIndex + 1) * heightPixels);
        }
        return pages;
    }
     ...
     */
    @NonNull
    public static ArrayList<Rect> computePdfPageBoundsFromCanvasView(
            @NonNull CanvasView view, float dpi) {
        ...
        //the list of bounds defining the size-constraints for the individual
        // pdf pages
        ArrayList<Rect> pages = new ArrayList<>();
        ...
        while (startIndex < boundsSortedY.size()) {
            ...
            currentPage.top = pages.isEmpty() ?
                    (int) boundsSortedY.get(startIndex).top :
                    pages.get(pages.size() - 1).bottom;
     ...
     */
    public static void renderCanvasViewContentsToPdfDocument(
            @NonNull CanvasView view, @NonNull ArrayList<Rect> pageBounds, @NonNull PdfDocument document) {
        for (Rect pageRect : pageBounds) {
            PdfDocument.PageInfo info =
                    new PdfDocument.PageInfo.Builder(pageRect.width(),
                            pageRect.height(), 1).create();
     ...
     */
    public static void exportPdfDocumentToFolder(
            @NonNull PdfDocument document, String folder, String filename) {
     ...
     * @param filename   Filename
     */
    public static void export(CanvasView canvasView, float dpi, String folder
            , String filename) {
        //compute the size for the pdf pages
        ArrayList<Rect> pages = computePdfPageBoundsFromCanvasView(canvasView
                , dpi);
    ...
    @Deprecated
    public static void export(CanvasView canvasView, float dpi, String folder
            , String filename, boolean enforceA4) {
        //compute the size for the pdf pages
        ArrayList<Rect> pages = enforceA4 ?
                computePdfPageBoundsFromCanvasViewStrict(canvasView, dpi,
                        PageSize.A4) :
                computePdfPageBoundsFromCanvasView(canvasView, dpi);
     ...
     */
    public static PdfDocument exportPdfDocument(CanvasView canvasView,
                                                float dpi, boolean enforceA4) {
        //compute the size for the pdf pages
        ArrayList<Rect> pages = enforceA4 ?
                computePdfPageBoundsFromCanvasViewStrict(canvasView, dpi,
                        PageSize.A4) :
