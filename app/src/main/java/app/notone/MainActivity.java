...
//region Persistence
        ...
        if (MainActivity.sRecentCanvases.size() != 0) {
            SharedPreferences sharedPreferences =
                    context.getSharedPreferences(
                            CanvasFragment.SHARED_PREFS_TAG, MODE_PRIVATE);
            SharedPreferences.Editor editor = sharedPreferences.edit();
            String recentCanvasesString = sRecentCanvases.toJson().toString();

            Log.d(TAG,
                    "onPause: storing recent files: " + recentCanvasesString);
            editor.putString(RECENT_CANVASES_LIST_PREF_KEY,
                    recentCanvasesString);
            editor.apply();
        }
    }
    ...
    private static void loadRecentCanvasesFromSharedPreferences(Context context) {
        SharedPreferences sharedPreferences =
                context.getSharedPreferences(CanvasFragment.SHARED_PREFS_TAG,
                        MODE_PRIVATE);
        String recentCanvasList =
                sharedPreferences.getString(RECENT_CANVASES_LIST_PREF_KEY, "");
        Log.d(TAG, "onResume: recentCanvasList: " + recentCanvasList +
                "replace old map: " + sRecentCanvases);
        try {
            sRecentCanvases =
                    RecentCanvases.fromJson(new JSONObject(recentCanvasList),
                            4);
...
//region Navigation Callbacks
     ...
     */
    /* Callback Functions */
    private final ActivityResultLauncher<String> mSavePdfDocument =
            ActivityResultLauncherProvider.getExportPdfActivityResultLauncher(this);
    private final ActivityResultLauncher<String> mNewCanvasFile =
            ActivityResultLauncherProvider.getNewCanvasFileActivityResultLauncher(this);
    private final ActivityResultLauncher<String[]> mOpenCanvasFile =
            ActivityResultLauncherProvider.getOpenCanvasFileActivityResultLauncher(this);
    public final ActivityResultLauncher<String> mSaveAsCanvasFile =
            ActivityResultLauncherProvider.getSaveAsCanvasFileActivityResultLauncher(this);
    ...
    /**
     * Back Navigation via Android Button and Back Arrow in toolbar gets
     * handled here
     */
    @Override
    public boolean onSupportNavigateUp() {
        Log.d(TAG, "onSupportNavigateUp");
        NavController navGraphController = findNavController(this,
                R.id.nav_main_host_fragment);
...
//region User Interface Setters
    ...
    public void updateRecentCanvasesExpListView() {
        ...
        Log.d(TAG, "updateExpListRecentFiles: " +
                Arrays.deepToString(recentCanvasList));
        mSimExpListView.invalidate();
        mSimExpListView.setAdapter((ExpandableListAdapter) null);
        mAdapter = RecentCanvasSimpleExpandableListAdapterBuilder.build(this,
                recentCanvasList);
     ...
     * used by the FAB
     */
    private void toggleToolbarVisibilityResponsively(AppBarLayout appBar,
                                                     FloatingActionButton fabToolbarVisibility) {
        if (mToolbarVisibility) {
            mToolbarVisibility = false;
            int offset = 39; // add a offset to the toolbar height, as its
            // partly hidden behind the android statusbar
...
//region ANDROID Lifecycle
     ...
     * .com/reference/androidx/navigation/ui/NavigationUI
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /* Set theme Preference on first start if it has never been set
        before */
        SharedPreferences sharedPreferences =
        ...
        AppCompatDelegate.setDefaultNightMode(darkModeActive ?
        ...
        mMainDrawerActivity = findViewById(R.id.drawer_activity_main); //
        // main base layout
        NavHostFragment navHostFragment =
                (NavHostFragment) getSupportFragmentManager().
                        findFragmentById(R.id.nav_main_host_fragment); // container of the fragments
        NavController navGraphController =
                navHostFragment.getNavController(); // nav_graph of the app
        mNavDrawerView = findViewById(R.id.navdrawercontainer_view); //
        // drawer menu container
        AppBarLayout toolbarContainer = findViewById(R.id.AppBar); // toolbar
        // container
        ...
        AppBarConfiguration appBarConfiguration =
                new AppBarConfiguration.Builder(navGraphController.getGraph())
                .setOpenableLayout(mMainDrawerActivity) // setDrawerLayout //
                        // define burger button for toplevel
                .build();
        NavigationUI.setupActionBarWithNavController(this, navGraphController
                , appBarConfiguration); // add titles and burger from
        // nav_graph to actionbar otherwise there will be the app title and
        // no burger!
        NavigationUI.setupWithNavController(mNavDrawerView,
                navGraphController); // this will call onNavDestination
        // (Selected||Changed) when a menu item is selected.
        ...
        mNavDrawerView.setNavigationItemSelectedListener(menuItem -> {
            ...
            switch (menuItem.getItemId()) {
                case R.id.open_server_file:
                    ...
                    return false;
                /* create a new file at a chosen uri and open it in the
                current canvas */
                case R.id.new_file:
                    ...
                    return false;
                /* chose a existing file with uri and open it in the current
                canvas */
                case R.id.open_file:
                    //CanvasFragment.sFlags.setOpenFile(true);
                ...
                case R.id.save_file:
                    ...
                    //Open the 'save as' dialog if the canvas does not have a
                    // uri.
                    if (CanvasFragment.sCanvasView.getCurrentURI().equals(Uri.parse(""))) {
                        /* save as to new uri (should not happen as there
                        shouldnt be any current canvases without uri) */
                        mSaveAsCanvasFile.launch("canvasFile.json");
                        return false;
                    }
                    FileManager.saveCanvasFileToUri(this,
                            CanvasFragment.sCanvasView.getCurrentURI());
                    //CanvasFileManager.safeSave(this, getApplicationContext
                    // (), CanvasFragment.sCanvasView.getCurrentURI(),
            ...
            /* forward click to the navigation controller if a navigation
            item is clicked*/
        ...
        Switch swAutoSave =
                mNavDrawerView.getMenu().
                        findItem(R.id.drawer_switch_autosave).
                        getActionView().
...
//        Not a MVP Feature
//        Switch swSync = mNavDrawerContainerNV.getMenu().findItem(R.id
//        .drawer_switch_sync).getActionView().findViewById(R.id.menu_switch);
//        swSync.setChecked(sharedPreferences.getBoolean("sync", false));
//        swSync.setOnCheckedChangeListener((compoundButton, b) -> spEditor
//        .putBoolean("sync", b).apply());
        ...
        mSimExpListView =
                mNavDrawerView.getMenu().
                        findItem(R.id.recent_files).
                        getActionView().
                        findViewById(R.id.exp_list_view);
        String[][] initialRecentFileNames =
                new String[][]{{"Default Value"}}; // string arrays for child
        // entries
        mAdapter = RecentCanvasSimpleExpandableListAdapterBuilder.build(this,
                initialRecentFileNames);
        mSimExpListView.setAdapter(mAdapter);

        mSimExpListView.collapseGroup(0);

        mSimExpListView.setOnGroupClickListener((parent, v, groupPosition,
                                                 id) -> {
            Log.d(TAG, "setOnGroupClickListener: " +
                    Arrays.toString(initialRecentFileNames[0]));
            if (!parent.isGroupExpanded(groupPosition)) {
                findViewById(R.id.exp_list_view).setLayoutParams(
                        new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, 310));
                Log.d(TAG, "setNavigationItemSelectedListener: changed list " +
                        "height");
            } else {
                findViewById(R.id.exp_list_view).setLayoutParams(
        ...
        mSimExpListView.setOnChildClickListener((parent, v, groupPosition,
                                                 childPosition, id) -> {
            String filename =
                    mAdapter.getChild(groupPosition, childPosition).
                            ...
                            replaceAll("\\{([A-Z])\\w+=", "").
                            replaceAll("\\}", "");
            Log.d(TAG, "mSimpleExpandableListView.setOnChildClickListener: " +
                    filename + sRecentCanvases.getByFilename(filename).mUri);
            FileManager.openCanvasFileFromUri(this,
                    sRecentCanvases.getByFilename(filename).mUri);
        ...
        FloatingActionButton fabToolbarVisibility =
                findViewById(R.id.button_toggle_toolbar);
        fabToolbarVisibility.setOnClickListener(view -> {
            toggleToolbarVisibilityResponsively(toolbarContainer,
                    fabToolbarVisibility);
        });



        /* Set toolbar padding depending on splitscreen state */
        // in multiwindow mode the screen has a clear border and cant be
        // behind system ui
        if (isInMultiWindowMode()) {
        ...
        navGraphController.addOnDestinationChangedListener((controller,
                                                            destination,
                                                            arguments) -> {
            TextView tvTitle =
                    ((TextView) findViewById(R.id.tv_fragment_title));
            LinearLayout viewCanvasToolsContainer =
                    findViewById(R.id.canvas_tools_container);
            ...
            switch (destination.getId()) {
                case R.id.canvas_fragment:
                    ...
                    tvTitle.setText(sCanvasName); // TODO replace with open
                    // document name
                ...
                case R.id.about_fragment:
                    ...
                    break;
                default:
                    throw new IllegalStateException("Destination changed to " +
                            "unexpected value: " + destination.getId());
            }
